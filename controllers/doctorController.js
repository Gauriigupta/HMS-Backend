...
const changeAvailability = async (req,res)=>{
    try {
        ...
        await doctorModel.findByIdAndUpdate(docId,{availability: !docData.available})
...
const appointmentComplete=async (req,res)=>{
    try {
        ...
        } else return res.json({success:false,message:"Mark failed"})
    } catch (error) {
        console.log(error)
        res.json({success:false,message:error.message})
    }
...
const appointmentCancel=async (req,res)=>{
    try {
        ...
        } else return res.json({success:false,message:"Cancellation failed"})
    } catch (error) {
        console.log(error)
        res.json({success:false,message:error.message})
    }
...
const updateDoctorProfile=async(req,res)=>{
    try {
        const {docId,fees,address,available,experience}=req.body
        await doctorModel.findByIdAndUpdate(docId,{fees,address,available,experience})
